# Mastery Exercises (required)

#### HTML

1. [Blog page](SW_HTML_BLOG.md)
1. [Banking application form](SW_HTML_BANKING_FORM.md)
1. [Personal bio site](SW_HTML_PERSONAL_BIO.md)

#### CSS

1. [Different styles for each article](SW_CSS_ARTICLE_STYLES.md)
1. [Using basic relationship selectors](SW_CSS_RELATIONSHIP_SELECTORS.md)
1. [Validate yor banking form](SW_CSS_BANKING_VALIDATE.md)
1. [Interactive navigation bar](SW_CSS_INTERACTIVE_NAVIGATION.md)
1. [Pseudo-classes](SW_CSS_PSEUDOCLASSES.md)
1. [Music History 01](SW_MUSIC_HISTORY_01.md)

#### JavaScript

1. [Colored reindeer](SW_JS_REINDEER.md)
1. [Boy bands & vegetables](SW_JS_BOYBANDS.md)
1. [Counting coins](SW_JS_COINS.md)
1. [Temperature converter](SW_JS_CONVERTER.md)
1. [Student grades](SW_JS_GRADES.md)
1. [Music History 02](SW_MUSIC_HISTORY_02.md)

# In-class Exercises

1. [Sonnet](SW_JS_SONNET.md)
